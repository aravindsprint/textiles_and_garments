...
#     Returns: dict with structure {work_order: {raw_item_code: finished_item_code}}
#     """
...
#     Formula: Process Loss Qty = (Sent Qty - Return Qty) - Received Qty
...
#     # Dictionary to aggregate data by work_order and FINISHED PRODUCT item_code
#     summary_data = {}
    
#     # Process Sent Details - Map raw items to finished product using Work Order mapping
...
#         # Skip if raw item is not in the required items list (unless it's the finished item itself)
#         if raw_item_code not in wo_item_mapping[work_order]['raw_items'] and raw_item_code != finished_item_code:
#             print(f"Raw item {raw_item_code} not found in Work Order {work_order} required items")
#             # Skip DYES% and CHEM% items silently
#             if not (raw_item_code and (raw_item_code.startswith("DYES") or raw_item_code.startswith("CHEM"))):
#                 print(f"Skipping sent item: {raw_item_code}")
#             continue
...
#         key = (work_order, finished_item_code)

#         if key not in summary_data:
#             summary_data[key] = {
#                 'work_order': work_order,
#                 'item_code': finished_item_code,  # Finished product
#                 'wo_qty': sent.wo_qty,
...
#         print(f"Added sent_qty {flt(sent.sent_qty)} for finished product {finished_item_code} (from raw: {raw_item_code})")

#     # Process Return Details - Map raw items to finished product using Work Order mapping
...
#             # Skip DYES% and CHEM% items silently
#             if not (raw_item_code and (raw_item_code.startswith("DYES") or raw_item_code.startswith("CHEM"))):
#                 print(f"Skipping return item: {raw_item_code}")
#             continue
...
#         print(f"Added return_qty {flt(ret.return_qty)} for finished product {finished_item_code} (from raw: {raw_item_code})")

#     # Process Received Details - This is already the finished product
#     for received in doc.work_order_received_details:
#         work_order = received.work_order
#         finished_item_code = received.item_code  # This is already the finished product
...
#             }
#         else:
#             summary_data[key]['uom'] = received.uom
#             summary_data[key]['wo_qty'] = received.wo_qty

#         summary_data[key]['received_qty'] += flt(received.received_qty)
#         print(f"Added received_qty {flt(received.received_qty)} for finished product {finished_item_code}")

#     # Calculate Process Loss for each item
...
#         # Process Loss Qty = Net Material Used - Received Qty
