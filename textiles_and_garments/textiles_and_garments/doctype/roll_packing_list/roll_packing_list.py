...
def create_manufacture_entry_from_roll_packing(doc):
    ...
    try:
        ...
        #   1. validate()        — items and fg_completed_qty have correct values ✓
        #   2. db_insert()       — DB is written with correct values ✓
        ...
        # yet have a Serial-Batch Bundle document, the handler can:
        #   (a) reset item.qty = 0 in-memory (bundle-qty mismatch resolution)
        #   (b) reset fg_completed_qty = 0 in-memory (bundle reconciliation side-effect)
        #
        # These mutations are NOT written back to DB — the DB still holds the correct
        # values from step 2.
        #
        # When submit() then calls save() → validate() on that same poisoned in-memory
        # object, it sees qty = 0 and/or fg_completed_qty = 0 and raises errors.
        #
        # Fix: discard the in-memory object entirely after insert and reload a clean
        # copy straight from DB. Then re-assert fg_completed_qty and from_bom since
        # submit()'s internal save() will re-validate those fields.
        #
        stock_entry_name = stock_entry.name
        stock_entry = frappe.get_doc("Stock Entry", stock_entry_name)

        # Re-assert fg_completed_qty and from_bom after reload.
        # submit() calls save() → validate() internally; if on_update zeroed these
        # fields in-memory (even if DB is correct), the reload restores DB values but
        # we re-assert here as a second defensive layer in case submit()'s own
        # validate logic recalculates them from BOM.
        stock_entry.fg_completed_qty = qty
        stock_entry.from_bom         = 0

        frappe.logger().info(
            f"Reloaded {stock_entry_name} from DB. "
            f"Re-asserted fg_completed_qty={qty}, from_bom=0. "
