...
class StockBalanceReport:
	...
	def prepare_new_data(self):
		...
		so_reserved_details = self.get_so_reserved_qty_details()
		...
		for _key, report_data in self.item_warehouse_map.items():
			...
			so_reserved_data = so_reserved_details.get((report_data.item_code, report_data.warehouse), {})
			report_data.update({
				"so_reserved_qty": so_reserved_data.get("qty", 0.0),
				"so_reserved_details": so_reserved_data.get("details", [])
			})
	...
	def get_so_reserved_qty_details(self) -> dict:
		"""Get Sales Order reserved quantities with details"""
		item_warehouse_pairs = list(self.item_warehouse_map.keys())
		...
		so_items = frappe.db.sql("""
			SELECT 
				soi.item_code,
				soi.warehouse,
				soi.qty - soi.delivered_qty as reserved_qty,
				so.name as sales_order,
				so.transaction_date as so_date,
				so.customer,
				soi.parent as custom_so_no
		...
		for so_item in so_items:
			key = (so_item.item_code, so_item.warehouse)
			if key not in result:
				result[key] = {"qty": 0.0, "details": []}
			
			result[key]["qty"] += so_item.reserved_qty
			result[key]["details"].append({
				"so_date": so_item.so_date,
				"sales_order": so_item.sales_order,
				"customer": so_item.customer,
				"qty": so_item.reserved_qty,
				"warehouse": so_item.warehouse,
				"custom_so_no": so_item.custom_so_no
			})
	...
	def apply_inventory_dimensions_filters(self, query, sle) -> str:
		"""Apply inventory dimensions filters to the query"""
	...
	def apply_warehouse_filters(self, query, sle) -> str:
		"""Apply warehouse filters to the query"""
	...
	def apply_items_filters(self, query, item_table) -> str:
		"""Apply item filters to the query"""
...
@frappe.whitelist()
def get_so_reserved_details(item_code, warehouse):
	"""Get SO reserved details for a specific item and warehouse"""
	so_items = frappe.db.sql("""
		SELECT 
			so.name as sales_order,
			so.transaction_date as so_date,
