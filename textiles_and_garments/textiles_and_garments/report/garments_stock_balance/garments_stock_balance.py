...
def execute(filters: StockBalanceFilter | None = None):
	return GarmentsStockBalanceReport(filters).run()


class GarmentsStockBalanceReport:
	...
	def prepare_opening_data_from_closing_balance(self) -> None:
		...
		for entry in res.data:
			...
			# Apply garments specific filters to opening data
			if not self.is_garments_item(entry):
				continue
	...
	def get_item_warehouse_map(self):
		...
		with frappe.db.unbuffered_cursor():
			...
			for entry in self.sle_entries:
				# Apply garments specific filters
				if not self.is_garments_item(entry):
					continue
	...
	def is_garments_item(self, entry) -> bool:
		"""
		Check if item meets garments criteria:
		- stock_uom is "Pcs"
		- item_code contains "/LA/" or "/PTS/"
		"""
		if entry.get('stock_uom') != 'Pcs':
			return False
			
		item_code = entry.get('item_code', '')
		if '/LA/' in item_code or '/PTS/' in item_code:
			return True
	...
	def prepare_stock_ledger_entries(self):
		...
		# Apply garments specific filters
		query = query.where(item_table.stock_uom == "Pcs")
		query = query.where(
			(item_table.item_code.like("%/LA/%")) | 
			(item_table.item_code.like("%/PTS/%"))
		)
	...
	def apply_items_filters(self, query, item_table) -> str:
		...
		if item_codes := self.filters.get("item_code"):
			# Only apply item code filter if user has specifically selected items
			# Otherwise, use our garments pattern filter
			query = query.where(item_table.name.isin(item_codes))
		else:
			# Apply garments pattern filter when no specific items are selected
			query = query.where(
				(item_table.item_code.like("%/LA/%")) | 
				(item_table.item_code.like("%/PTS/%"))
			)
