...
def create_quotation_from_deal(crm_deal: str, organization: str | None = None):
    ...
    # ── 1. Resolve Customer from CRM Deal ─────────────────────────────────────
    customer_name = frappe.db.get_value("Customer", {"crm_deal": crm_deal}, "name")
    if not customer_name:
        customer_name = getattr(deal, "customer", None)
    if not customer_name:
        frappe.throw(
            f"No Customer found linked to CRM Deal {crm_deal}. "
            "Please convert the deal to a customer first."
        )
